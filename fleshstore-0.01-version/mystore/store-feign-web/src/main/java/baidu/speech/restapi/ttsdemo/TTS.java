...
public class TTS {
    ...
    private static String cuid = "16206017";
    public static int vol =1;                   //音量，取值0-15，默认为5中音量
    private static int ctp = 1;                 //	客户端类型选择，web端填写固定值1
    public static int per = 4;                  // 发音人选择, 0为普通女声，1为普通男生，3为情感合成-度逍遥，4为情感合成-度丫丫，默认为普通女声
    public static int spd = 5;                  //语速，取值0-15，默认为5中语速
    public static int pit = 10;                 //音调，取值0-15，默认为5中语调
    private static int aue = 6;                 //3为mp3格式(默认)； 4为pcm-16k；5为pcm-8k；6为wav（内容同pcm-16k）; 注意aue=4或者6是语音识别要求的格式，但是音频内容不是语音识别要求的自然人发音，所以识别效果会受影响。
    ...
    public static void outputMP3(String text,String path) throws IOException, DemoException {
